...
/*
  PWMServoFast.cpp - Hardware Servo Timer Library //Modified by Marek Travnikar
...
uint8_t PWMServo::attach(int pinArg, int min, int max, int servFreq) //ServFreq not implemented for AVR
{
  #ifdef SERVO_PIN_C
...
uint8_t PWMServo::attach(int pinArg, int min, int max, int servFreq)
{
	//Serial.printf("attach, pin=%d, min=%d, max=%d\n", pinArg, min, max);
	if (pinArg < 0 || pinArg >= NUM_DIGITAL_PINS) return 0;
	if (!digitalPinHasPWM(pinArg)) return 0;
	pin = pinArg;
	analogWriteFrequency(pin, servFreq); //Changed to allow a change in PWM freqnecy
	min16 = min >> 4;
	max16 = max >> 4;
    servFreqS = servFreq;
	angle = NO_ANGLE;
...
void PWMServo::write(int angleArg)
{
	...
	if (angleArg > 180) angleArg = 180;
	angle = angleArg;
	//uint32_t us = (((max16 - min16) * 932 * angle * servFreqS) >> 11) + (min16 << 12); // us*256
    //uint32_t us = (((max16 - min16) * 46603 * angle) >> 11) + (min16 << 12); // us*256 (orig)
    uint32_t us = (((max16 - min16) * 48867 * angle) >> 12) + (min16 << 11); // ~us*134.2
	//uint32_t duty = (us * 17*servFreqS) >> 20;
    uint32_t duty = (us * servFreqS) >> 15;
    //uint32_t duty = (us * 3355) >> 22; (orig)
	//float usec = (float)((max16 - min16)<<4) * ((float)angle / 180.0f) + (float)(min16<<4);
	//uint32_t duty = (int)(usec / 20000.0f * 4096.0f);
	//Serial.printf("angle=%d, us=%.2f, duty=%d, min=%d, max=%d\n", angle, (float)us / 134.2f, duty, min16<<4, max16<<4);
